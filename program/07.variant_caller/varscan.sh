...
--p-value 0.05 \
--output-vcf 1 \
--vcf-sample-list samples_name_vs.list | \
bgzip -f -c -@ $THREADS > $VCF_TMP/"$ALIGNER"_"$CALLER"_raw.vcf.gz
wait
...
# --output-vcf 1 \
# --vcf-sample-list samples_name_vs.list | \
# bgzip -f -c -@ $THREADS > $VCF_TMP/"$ALIGNER"_"$CALLER"_raw_indel.vcf.gz
# wait
